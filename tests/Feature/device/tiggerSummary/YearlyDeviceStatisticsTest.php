...
class YearlyDeviceStatisticsTest extends TestCase
{
    use RefreshDatabase;

    /** @test */
    public function test_user_can_get_device_yearly_updates()
    {
        ...
        $response = $this->getJson(route('yearly.triggers', $device))->json();

        $this->assertTrue(sizeof($response) >= 365);
    }
}
