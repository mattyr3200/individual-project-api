...
class MonthlyDeviceStatisticsTest extends TestCase
{
    use RefreshDatabase;

    /** @test */
    public function test_user_can_get_device_monthly_updates()
    {
        ...
        $response = $this->getJson(route('monthly.triggers', $device))->json();


        $this->assertTrue(sizeof($response) >= 28);
    }
}
